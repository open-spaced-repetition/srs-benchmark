...
The image below shows the p-values obtained by running the Wilcoxon signed-rank test on the RMSE (bins) of all pairs of algorithms. Red means "the row algorithm performs worse than the corresponding column algorithm", green is the opposite. Top row and rightmost column are green because FSRS-4.5 outperforms all other algorithms, bottom row and leftmost column are red because HLR performs worse than any other algorithm.
