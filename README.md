...
The image below shows the p-values obtained by running the Wilcoxon signed-rank test on the RMSE (bins) of all pairs of algorithms. Red means that the row algorithm performs worse than the corresponding column algorithm, green means the row algorithm performs better than the corresponding column algorithm. Top row is green because FSRS-4.5 outperforms all other algorithms, bottom row is red because HLR performs worse than any other algorithm.
