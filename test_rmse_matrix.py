...
def test_calibrated():
    df = _df(
        elapsed_days=[5, 5], i=[2, 2], rmse_bins_lapse=[0, 0], y=[1, 0], p=[0.5, 0.5]
    )
...
def test_uniform_gap():
    df = _df(
        elapsed_days=[5, 5], i=[2, 2], rmse_bins_lapse=[0, 0], y=[1, 1], p=[0.9, 0.9]
    )
...
def test_unweighted_bins():
    df = _df(
        elapsed_days=[2, 100], i=[2, 5], rmse_bins_lapse=[0, 1], y=[1, 0], p=[0.9, 0.2]
    )
...
def test_lapse_zero_bin():
    df = _df(
        elapsed_days=[5, 5], i=[2, 2], rmse_bins_lapse=[0, 3], y=[1, 0], p=[1.0, 0.0]
    )
    assert rmse_matrix(df) == pytest.approx(0.0)
