...
for file in tqdm(list(Path(dataset_path).iterdir())):
    ...
    for window_size in window_sizes:
        ...
        for j in range(len(day_review_counts) - window_size + 1):
            window_sum = sum(day_review_counts.get(k, 0) for k in range(j, j + window_size))
...
for idx, window_size in enumerate(window_sizes):
    axs[idx].hist(all_averages[window_size], bins=100, alpha=0.7, color='blue', edgecolor='black')
    axs[idx].axvline(pd.Series(all_averages[window_size]).quantile(0.5), color='red', linestyle='dashed', linewidth=1, label=f'50th Percentile - {pd.Series(all_averages[window_size]).quantile(0.5):.2f} reviews/day')
    axs[idx].axvline(pd.Series(all_averages[window_size]).quantile(0.75), color='green', linestyle='dashed', linewidth=1, label=f'75th Percentile - {pd.Series(all_averages[window_size]).quantile(0.75):.2f} reviews/day')
    axs[idx].axvline(pd.Series(all_averages[window_size]).quantile(0.9), color='orange', linestyle='dashed', linewidth=1, label=f'90th Percentile - {pd.Series(all_averages[window_size]).quantile(0.9):.2f} reviews/day')
    axs[idx].axvline(pd.Series(all_averages[window_size]).quantile(0.95), color='purple', linestyle='dashed', linewidth=1, label=f'95th Percentile - {pd.Series(all_averages[window_size]).quantile(0.95):.2f} reviews/day')
    axs[idx].axvline(pd.Series(all_averages[window_size]).quantile(0.99), color='brown', linestyle='dashed', linewidth=1, label=f'99th Percentile - {pd.Series(all_averages[window_size]).quantile(0.99):.2f} reviews/day')
    axs[idx].axvline(pd.Series(all_averages[window_size]).quantile(1.0), color='black', linestyle='dashed', linewidth=1, label=f'100th Percentile - {pd.Series(all_averages[window_size]).quantile(1.0):.2f} reviews/day')
