...
import numpy as np
import torch
from rwkv.config import DAY_OFFSET_ENCODE_PERIODS, ID_ENCODE_DIMS, ID_SPLIT, RWKV_SUBMODULES
    ...
    with torch.no_grad():
        ...
        def add_encodings(card_features, day_offsets, day_offsets_first, ids):
            def generate_id_encoding(submodule):
                ...
                return torch.randint(low=0, high=ID_SPLIT, size=(ENCODE_DIM,), device=card_features.device, requires_grad=False).to(card_features.dtype) - ((ID_SPLIT - 1) / 2)
            ...
            for period in DAY_OFFSET_ENCODE_PERIODS:
                ...
                f = 2 * np.pi / period
                encodings_sin = torch.sin(f * ((baseline + day_offsets) % period)).to(card_features.dtype)
                encodings_cos = torch.cos(f * ((baseline + day_offsets) % period)).to(card_features.dtype)
                encodings = torch.stack((encodings_sin, encodings_cos), dim=-1)
                gather.append(encodings)
                encodings_first_sin = torch.sin(f * ((baseline + day_offsets_first) % period)).to(card_features.dtype)
                encodings_first_cos = torch.cos(f * ((baseline + day_offsets_first) % period)).to(card_features.dtype)
                encodings_first = torch.stack((encodings_first_sin, encodings_first_cos), dim=-1)
                gather.append(encodings_first)

            return torch.cat(gather, dim=-1)

        card_features_with_ids = [add_encodings(data.card_features, data.day_offsets, data.day_offsets_first, data.ids) for data in data_list]
        start_tensor = torch.cat([torch.nn.functional.pad(card_features, (0, 0, 0, global_T - card_features.size(0))) for card_features in card_features_with_ids], dim=0)
        ...
        boundary_offset = 0
        current_locs_list = [i*global_T + torch.arange(0, data.card_features.size(0), 1, dtype=torch.long) for i, data in enumerate(data_list)]
        ...
        if target_len is None:
            ...
            splits = greedy_splits(data_list, factor=target_len*(1 + factor)/data_list_t_sum - 1)
        sub_gather = []
        sub_skip_gather = []
        ...
        for submodule_name, _ in DEFAULT_ANKI_RWKV_CONFIG.modules:
            ...
            all_offset = 0
            next_locs_list = [np.zeros(data.card_features.size(0), dtype=np.int64) for data in data_list]
            gather_lens = []
            gather = []
            skip_gather = []
            time_shift_gather = []
            for split_i in range(len(split)):
                ...
                for data_i, (data, current_locs) in enumerate(zip(data_list, current_locs_list)):
                    ...
                    for module_data_i, (data_split_B, data_split_len) in enumerate(zip(module_data.split_B, module_data.split_len)):
                        if l < data_split_len and data_split_len <= r:
                            from_slice = module_data.from_perm[boundaries[module_data_i] : boundaries[module_data_i + 1]]
                            take_from = torch.index_select(current_locs, dim=0, index=from_slice).view(data_split_B, data_split_len)

                            # Maybe random instead of 0 padding to reduce collisions
                            take_from = torch.nn.functional.pad(take_from, (0, r - data_split_len), mode='constant', value=-1)
                            take_list.append(take_from)

                            skip = torch.index_select(data.skips, dim=0, index=from_slice).view(data_split_B, data_split_len)
                            skip_arr = skip.numpy()
                            time_shift_select = np.zeros((data_split_B, data_split_len))
                            assert (skip_arr[0] == False).any(), "Cannot skip the start; otherwise we need to be careful for consecutive Trues at the start."
                            ...
                            skip = torch.nn.functional.pad(skip, (0, r - data_split_len), mode='constant', value=True)
                            skip_list.append(skip)
                            time_shift_select = torch.nn.functional.pad(torch.tensor(time_shift_select, dtype=torch.int32, device=skip.device), (0, r - data_split_len), mode='constant', value=0)
                            time_shift_list.append(time_shift_select)

                            for seq_unpadded in from_slice.view(data_split_B, data_split_len):
                ...
                time_shift_gather.append(torch.cat(time_shift_list, dim=0).flatten().long())
        ...
        def pad_labels(labels):
            return torch.nn.functional.pad(labels, (0, 0, 0, global_T - labels.size(0)), mode='constant', value=0)
        padded_labels = torch.stack(list(map(lambda data: pad_labels(data.global_labels), data_list)))

        def pad_review_ths(labels):
            return torch.nn.functional.pad(labels, (0, global_T - labels.size(0)), mode='constant', value=-1)
        padded_label_review_th = torch.stack(list(map(lambda data: pad_review_ths(data.label_review_ths), data_list)))
        return PreparedBatch(num_data=len(data_list), 
                             start=start_tensor, 
...
def greedy_splits(data_list: list[RWKVSample], factor, allowed_excess_in_one_step=20000):
    ...
    for submodule in RWKV_SUBMODULES:
        ...
        while l < len(lens):
            ...
            while r + 1 < len(lens):
                ...
                if factor * next_used >= next_waste and extra_waste <= allowed_excess_in_one_step:
                    used = next_used
                    waste = next_waste
                    r += 1
                else:
