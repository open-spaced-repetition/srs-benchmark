...
class RWKV7RNN(torch.nn.Module):
    def __init__(self, config: RWKV7Config):
        super().__init__()
        self.blocks = torch.nn.ModuleList([RWKV7RNNLayer(config, layer_id) for layer_id in range(config.n_layers)])
...
class RWKV7RNNLayer(torch.nn.Module):
    ...
    def forward(self, in_BC, v0_BC, state):
        if state is None:
            state = None, None
        time_state, channel_state = state
        x_BC, v0_BC, time_state = self.time_mixer(in_BC=in_BC, v0_BC=v0_BC, state=time_state)
...
class RWKV7RNNTimeMixer(torch.nn.Module):
    ...
    def forward(self, in_BC, v0_BC, state):
        ...
        if state is None:
            x_shift_B1C = x_B1C
            state_B1HKK = torch.zeros(B, 1, H, K, K, dtype=torch.float32, device=in_BC.device)
        else:
            x_shift_B1C, state_B1HKK = state

        rkvdag_6B1C = torch.lerp(x_B1C.unsqueeze(0), x_shift_B1C.unsqueeze(0), self.rkvdag_lerp)
        r_B1C, k_B1C, v_B1C, d_B1C, a_B1C, g_B1C, k_scale_B1C, v_scale_B1C = rkvdag_6B1C.unbind(dim=0)
        ...
        k_B1HK = k_scale_B1H.unsqueeze(-1) * torch.nn.functional.normalize(k_B1C.view(B, 1, H, K), dim=-1, p=2.0)
        r_B1HK = r_B1C.view(B, 1, H, K)
        v_B1HK = v_scale_B1H.unsqueeze(-1) * torch.nn.functional.normalize(v_B1C.view(B, 1, H, K), dim=-1, p=2.0)
        w_B1HK = w_B1C.view(B, 1, H, K)
        a_B1HK = a_B1C.view(B, 1, H, K)
        k_deformed_B1HK = k_B1HK
        k_B1HK = k_B1HK * a_B1HK
        # k_deformed_B1HK = torch.nn.functional.normalize(k_deformed_B1C.view(B, 1, H, K), dim=-1, p=2.0)

        out_BHK, next_state_BHKK = single_timestep(r_B1HK.float().squeeze(1),
        ...
        bonus_B1C = ((r_B1HK * self.bonus * k_B1HK).sum(dim=-1, keepdim=True) * v_B1HK).view(B, 1, C)
        out_B1C = self.W_o(g_B1C * (out_B1C + bonus_B1C))
        return (in_B1C + out_B1C).squeeze(1), v0_BC, (x_layer_norm_B1C, next_state_BHKK.unsqueeze(1))
