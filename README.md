...
| DASH | 0.375±0.0030 | 0.077±0.0010 | 9 |
| FSRS-4.5 (default parameters) | 0.376±0.0033 | 0.095±0.0011 | 0 |
| DASH[MCM] | 0.381±0.0029 | 0.085±0.0011 | 9 |
...
The image below shows the p-values obtained by running the Wilcoxon signed-rank test on the log loss of all pairs of algorithms. Red means that the row algorithm performs worse than the corresponding column algorithm, and green means that the row algorithm performs better than the corresponding column algorithm. The top row is green because FSRS-4.5 outperforms all other algorithms; the bottom row is red because HLR performs worse than any other algorithm.
