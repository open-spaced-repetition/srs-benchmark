...
from llm.rwkv.srs_model import AnkiRWKV, extract_p
from llm.rwkv.srs_model_rnn import AnkiRWKVRNN, get_df_sequentially
from llm.data_processing import add_queries, add_segment_features, create_sample, get_rwkv_data
import torch

DEVICE = "cuda"
RNN_DEVICE = "cpu"
DTYPE = torch.float32  # For small df sizes where there is no rwkv time-parallelism, torch.bfloat16 should work but it doesn't. There is likely a small bug somewhere.
DROPOUT = 0.0
CARD_CONFIG = RWKV7Config(d_model=32*1, n_heads=1, n_layers=2, channel_mixer_factor=3, decay_lora=8, a_lora=8, v0_mix_amt_lora=8, gate_lora=16, k_scale_lora=4, v_scale_lora=4, dropout=DROPOUT)
NOTE_CONFIG = RWKV7Config(d_model=32*1, n_heads=1, n_layers=2, channel_mixer_factor=3, decay_lora=8, a_lora=8, v0_mix_amt_lora=8, gate_lora=16, k_scale_lora=4, v_scale_lora=4, dropout=DROPOUT)
DECK_CONFIG = RWKV7Config(d_model=32*1, n_heads=1, n_layers=2, channel_mixer_factor=3, decay_lora=8, a_lora=8, v0_mix_amt_lora=8, gate_lora=16, k_scale_lora=4, v_scale_lora=4, dropout=DROPOUT)
PRESET_CONFIG = RWKV7Config(d_model=32*1, n_heads=1, n_layers=2, channel_mixer_factor=3, decay_lora=8, a_lora=8, v0_mix_amt_lora=8, gate_lora=16, k_scale_lora=4, v_scale_lora=4, dropout=DROPOUT)
GLOBAL_CONFIG = RWKV7Config(d_model=32*1, n_heads=1, n_layers=2, channel_mixer_factor=3, decay_lora=8, a_lora=8, v0_mix_amt_lora=8, gate_lora=16, k_scale_lora=4, v_scale_lora=4, dropout=DROPOUT)
...
class Test(unittest.TestCase):
    def test_equivalence(self):
        ...
        with torch.no_grad():
            ...
            base.eval()
            rnn_master = AnkiRWKVRNN(card_rwkv_config=CARD_CONFIG, note_rwkv_config=NOTE_CONFIG, deck_rwkv_config=DECK_CONFIG, preset_rwkv_config=PRESET_CONFIG, global_rwkv_config=GLOBAL_CONFIG).to(RNN_DEVICE)
            rnn = AnkiRWKVRNN(card_rwkv_config=CARD_CONFIG, note_rwkv_config=NOTE_CONFIG, deck_rwkv_config=DECK_CONFIG, preset_rwkv_config=PRESET_CONFIG, global_rwkv_config=GLOBAL_CONFIG).selective_cast(DTYPE).to(RNN_DEVICE)
            rnn.copy_downcast_(rnn_master, dtype=DTYPE)
            rnn.load_state_dict(base.state_dict())

            for user_id in list(range(1, 2)):
                df = get_rwkv_data(user_id, equalize_review_ths=[])
                # df = df.head(min(len(df), 200))
                print("Test", user_id, len(df))
                sample = create_sample(user_id, df.copy(), equalize_review_ths=[], dtype=DTYPE, device="cpu")
                stats = base.get_loss(prepare([sample]).to(DEVICE))
                base_ahead_p, base_imm_p, _ = extract_p(stats)

                # print("base_ahead_p", base_ahead_p)
                # print("base_imm_p", base_imm_p)

                rnn_df = add_queries(add_segment_features(df), [], 0, 1)
                print("start rnn")
