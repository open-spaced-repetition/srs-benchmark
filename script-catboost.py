...
def predict(w_list, trainsets, testsets, file=None):
    p = []
    y = []
    save_tmp = [] if file else None

    for i, (w, trainset, testset) in enumerate(zip(w_list, trainsets, testsets)):
        my_collection = Collection(w)
        trainset["stability"], trainset["difficulty"] = my_collection.batch_predict(
            trainset
        )
        trainset["pf"] = power_forgetting_curve(
            trainset["delta_t"], trainset["stability"]
        )

        testset["stability"], testset["difficulty"] = my_collection.batch_predict(
            testset
        )
        testset["pf"] = power_forgetting_curve(testset["delta_t"], testset["stability"])

        catboost_traindata = trainset[["delta_t", "pf", "stability", "difficulty"]]
        train_pool = Pool(
            catboost_traindata,
            label=trainset["y"].astype(int),
        )
        catboost_evaldata = testset[["delta_t", "pf", "stability", "difficulty"]]
        eval_pool = Pool(
            catboost_evaldata,
            label=testset["y"].astype(int),
        )

        model = CatBoostClassifier(
            verbose=False, allow_writing_files=False, has_time=True
        )
        f = model.fit(train_pool, eval_set=eval_pool, early_stopping_rounds=100)
        # print(f"Model accuracy on eval data: {f.score(eval_pool):.5%}")
        # print(f.get_feature_importance(prettified=True))

        prob = f.predict_proba(eval_pool)
        testset["p"] = prob[:, np.where(f.classes_ == 1)[0][0]]
...
def process(file):
    ...
    if do_fullinfo_stats:
        ...
        for i in range(len(w_list)):
            p, y, evaluation = predict(
                [w_list[i]], [trainsets[i]], [testsets[i]], file=file
            )
            all_p.append(p)
            all_y.append(y)
        ...
        for i in range(len(w_list)):
            p, y, evaluation = predict(
                [w_list[i]], [trainsets[i]], [trainsets[i]], file=file
            )
            all_p.append(p)
            all_y.append(y)
            all_evaluation.append(evaluation)
        ...
        p, y, evaluation = predict(w_list, trainsets, testsets, file=file)
        last_y = y
